...
namespace Org.BouncyCastle.Security
{
    ...
    {
        private static readonly IDictionary _kgAlgorithms = Platform.CreateHashtable();
        private static readonly IDictionary _kpgAlgorithms = Platform.CreateHashtable();
        private static readonly IDictionary _defaultKeySizes = Platform.CreateHashtable();
        ...
        private static void AddDefaultKeySizeEntries(int size, params string[] algorithms)
        {
            ...
            {
                _defaultKeySizes.Add(algorithm, size);
            }
        }
        ...
        {
            _kgAlgorithms[canonicalName] = canonicalName;

            foreach (var alias in aliases)
            {
                _kgAlgorithms[alias.ToString()] = canonicalName;
            }
        }

        private static void AddKpgAlgorithm(string canonicalName, params object[] aliases)
        {
            _kpgAlgorithms[canonicalName] = canonicalName;

            foreach (var alias in aliases)
            {
                _kpgAlgorithms[alias.ToString()] = canonicalName;
            }
        }
        ...
        private static void AddHMacKeyGenerator(string algorithm, params object[] aliases)
        {
            ...
            _kgAlgorithms[mainName] = mainName;
            _kgAlgorithms["HMAC-" + algorithm] = mainName;
            _kgAlgorithms["HMAC/" + algorithm] = mainName;

            foreach (object alias in aliases)
            {
                _kgAlgorithms[alias.ToString()] = mainName;
            }
        }
        ...
        public static IAsymmetricCipherKeyPairGenerator GetKeyPairGenerator(PublicKeyAlgorithmTag algorithm)
        {
            return GetKeyPairGenerator(algorithm.ToString());
        }
        ...
        {
            if (!_defaultKeySizes.Contains(canonicalName))
                return -1;

            return (int)_defaultKeySizes[canonicalName];
        }
    }
}
