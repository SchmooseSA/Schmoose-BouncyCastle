...
namespace Org.BouncyCastle.Pkix
{
	...
	public class PkixCertPathValidatorUtilities
	{
			...
			ISet			trustAnchors)
		{
			...
			IAsymmetricKeyParameter trustPublicKey = null;
...
//							baseDN = nsull;
		...
		private static IBigInteger GetSerialNumber(
			object cert)
		{
			if (cert is X509Certificate)
			{
...
//					pq.Add(PolicyQualifierInfo.GetInstance(Asn1Object.FromByteArray(ae.GetEncoded())));
		...
		*/
		internal static IAsymmetricKeyParameter GetNextWorkingKey(
			IList	certs,
			int		index)
		{
			...
			IAsymmetricKeyParameter pubKey = cert.GetPublicKey();
