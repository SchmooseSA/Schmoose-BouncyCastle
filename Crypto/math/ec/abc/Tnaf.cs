...
namespace Org.BouncyCastle.Math.EC.Abc
{
	...
	*/
	internal class Tnaf
	{
        ...
        private static readonly IBigInteger MinusThree = BigInteger.Three.Negate();
        private static readonly IBigInteger Four = BigInteger.ValueOf(4);
		...
		*/
        public static IBigInteger GetTw(sbyte mu, int w) 
		{
			if (w == 4)
			{
				if (mu == 1)
            ...
            IBigInteger[] us = GetLucas(mu, w, false);
            IBigInteger twoToW = BigInteger.Zero.SetBit(w);
            IBigInteger u1invert = us[1].ModInverse(twoToW);
            IBigInteger tw;
