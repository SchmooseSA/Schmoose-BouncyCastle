...
namespace Org.BouncyCastle.Math.EC
{
	...
	public class FpFieldElement
		: ECFieldElement
	{
        private readonly IBigInteger q, x;
		...
		public override ECFieldElement Sqrt()
		{
			...
			// p mod 4 == 1
            IBigInteger qMinusOne = q.Subtract(BigInteger.One);

            IBigInteger legendreExponent = qMinusOne.ShiftRight(1);
			if (!(x.ModPow(legendreExponent, q).Equals(BigInteger.One)))
				return null;

            IBigInteger u = qMinusOne.ShiftRight(2);
            IBigInteger k = u.ShiftLeft(1).Add(BigInteger.One);

            IBigInteger Q = this.x;
            IBigInteger fourQ = Q.ShiftLeft(2).Mod(q);
			...
			do
			{
                ...
                IBigInteger[] result = fastLucasSequence(q, P, Q, k);
				U = result[0];
				V = result[1];
...
//			IBigInteger fourX = x.ShiftLeft(2);
//
//			IBigInteger r;
//			do
//			{
...
//
//			IBigInteger n1 = qMinusOne.ShiftRight(2);
//			IBigInteger n2 = n1.Add(BigInteger.One);
//
//			IBigInteger wOne = WOne(r, x, q);
//			IBigInteger wSum = W(n1, wOne, q).Add(W(n2, wOne, q)).Mod(q);
//			IBigInteger twoR = r.ShiftLeft(1);
//
//			IBigInteger root = twoR.ModPow(q.Subtract(BigInteger.Two), q)
//				.Multiply(x).Mod(q)
//				.Multiply(wSum).Mod(q);
//
...
//		private static IBigInteger W(IBigInteger n, IBigInteger wOne, IBigInteger p)
//		{
...
//			if (isEven)
//			{
//				IBigInteger w = W(n, wOne, p);
//				return w.Multiply(w).Subtract(BigInteger.Two).Mod(p);
//			}
//			IBigInteger w1 = W(n.Add(BigInteger.One), wOne, p);
//			IBigInteger w2 = W(n, wOne, p);
//			return w1.Multiply(w2).Subtract(wOne).Mod(p);
//		}
//
//		private IBigInteger WOne(IBigInteger r, IBigInteger x, IBigInteger p)
//		{
//			return r.Multiply(r).Multiply(x.ModPow(q.Subtract(BigInteger.Two), q)).Subtract(BigInteger.Two).Mod(p);
//		}

        private static IBigInteger[] fastLucasSequence(
            IBigInteger p,
            IBigInteger P,
            IBigInteger Q,
            IBigInteger k)
		{
            ...
            IBigInteger Uh = BigInteger.One;
            IBigInteger Vl = BigInteger.Two;
            IBigInteger Vh = P;
            IBigInteger Ql = BigInteger.One;
            IBigInteger Qh = BigInteger.One;
