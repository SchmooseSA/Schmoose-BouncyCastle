		...
		: IAsymmetricBlockCipher
	{
		...
		*/
			...
			int		length)
		{
			...
			IBigInteger p = key.Parameters.P;
			...
			if (key is ElGamalPrivateKeyParameters) // decryption
			{
				...
				IBigInteger m = gamma.ModPow(p.Subtract(BigInteger.One).Subtract(priv.X), p).Multiply(phi).Mod(p);
			...
			else // encryption
			{
				...
				IBigInteger g = key.Parameters.G;
				IBigInteger gamma = g.ModPow(k, p);
				IBigInteger phi = tmp.Multiply(pub.Y.ModPow(k, p)).Mod(p);
