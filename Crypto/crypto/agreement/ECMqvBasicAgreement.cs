...
namespace Org.BouncyCastle.Crypto.Agreement
{
		...
		: IBasicAgreement
	{
			...
			ECPublicKeyParameters	Q2V)
		{
            ...
            IBigInteger powE = BigInteger.One.ShiftLeft(e);
			...
			IBigInteger x = q.X.ToBigInteger();
			IBigInteger xBar = x.Mod(powE);
			IBigInteger Q2UBar = xBar.SetBit(e);
			IBigInteger s = d1U.D.Multiply(Q2UBar).Mod(n).Add(d2U.D).Mod(n);

			IBigInteger xPrime = Q2V.Q.X.ToBigInteger();
			IBigInteger xPrimeBar = xPrime.Mod(powE);
			IBigInteger Q2VBar = xPrimeBar.SetBit(e);

			IBigInteger hs = parameters.H.Multiply(s).Mod(n);
