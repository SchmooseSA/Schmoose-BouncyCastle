...
namespace Org.BouncyCastle.Bcpg.OpenPgp.Tests
{
    ...
    public class PgpKeyRingTest
        : SimpleTest
    {
        private static readonly byte[] _pub1 = Base64.Decode(
        ...
        private static readonly byte[] _sec1 = Base64.Decode(
...
//		private static readonly char[] pass1 = "qwertzuiop".ToCharArray();
        ...
        private static readonly byte[] _pub2 = Base64.Decode(
        ...
        private static readonly byte[] _sec2 = Base64.Decode(
        ...
        private static readonly char[] _sec2Pass1 = "sandhya".ToCharArray();
        private static readonly char[] _sec2Pass2 = "psai".ToCharArray();

        private static readonly byte[] _pub3 = Base64.Decode(
        ...
        private static readonly byte[] _sec3 = Base64.Decode(
        ...
        private static readonly char[] _sec3Pass1 = "123456".ToCharArray();

        //
        // GPG comment packets.
        //
        private static readonly byte[] _sec4 = Base64.Decode(
        ...
        //
        private static readonly byte[] _pub5 = Base64.Decode(
        ...
        //
        private static readonly byte[] _pub6 = Base64.Decode(
...
//		private static readonly byte[] pub7check = Base64.Decode("f/YQ");
        ...
        private static readonly byte[] _pub8 = Base64.Decode(
        ...
        private static readonly byte[] _sec8 = Base64.Decode(
        ...
        private static readonly char[] _sec8Pass = "qwertyui".ToCharArray();

        private static readonly byte[] _sec9 = Base64.Decode(
        ...
        private static readonly char[] _sec9Pass = "foo".ToCharArray();

        // version 4 keys with expiry dates
        private static readonly byte[] _pub10 = Base64.Decode(
        ...
        private static readonly byte[] _sec10 = Base64.Decode(
...
//		private static readonly char[] sec10pass = "test".ToCharArray();
        ...
        private static readonly byte[] _subKeyBindingKey = Base64.Decode(
...
//		private static readonly byte[] subKeyBindingCheckSum = Base64.Decode("3HU+");
        ...
        //
        private static readonly byte[] _rewrapKey = Base64.Decode(
        ...
        private static readonly char[] _rewrapPass = "voltage123".ToCharArray();

        private static readonly byte[] _pubWithX509 = Base64.Decode(
        ...
        private static readonly byte[] _secWithPersonalCertificate = Base64.Decode(
        ...
        private void CheckPublicKeyRingWithX509(byte[] keyRing)
        {
            ...
            if (en.MoveNext())
            {
                ...
                if (sEn.MoveNext())
                {
                    var sig = (PgpSignature) sEn.Current;
        ...
        public void GenerateSha1Test()
        {
            ...
            var elParams = new ElGamalParameters(p, g);
            var elKgp = new ElGamalKeyGenerationParameters(new SecureRandom(), elParams);
            elgKpg.Init(elKgp);
            ...
            var dsaKeyPair = new PgpKeyPair(PublicKeyAlgorithmTag.Dsa, dsaKp, DateTime.UtcNow);
            var elgKeyPair = new PgpKeyPair(PublicKeyAlgorithmTag.ElGamalEncrypt, elgKp, DateTime.UtcNow);

            var keyRingGen = new PgpKeyRingGenerator(PgpSignature.PositiveCertification, dsaKeyPair,
                                                     "test", SymmetricKeyAlgorithmTag.Aes256, passPhrase, true, null,
        ...
        public void GenerateTest()
        {
            ...
            pGen.Init(512, 80, new SecureRandom());
            DsaParameters dsaParams = pGen.GenerateParameters();
            var dsaKgp = new DsaKeyGenerationParameters(new SecureRandom(), dsaParams);
            ...
            var keyRingGen = new PgpKeyRingGenerator(PgpSignature.PositiveCertification, dsaKeyPair,
                                                     "test", SymmetricKeyAlgorithmTag.Aes256, passPhrase, null, null,
                                                     new SecureRandom());
        ...
        public void InsertMasterTest()
        {
            ...
            // this is quicker because we are using pregenerated parameters.
            //
            IAsymmetricCipherKeyPair rsaKp = rsaKpg.GenerateKeyPair();
            var rsaKeyPair1 = new PgpKeyPair(PublicKeyAlgorithmTag.RsaGeneral, rsaKp, DateTime.UtcNow);

            rsaKp = rsaKpg.GenerateKeyPair();
            var rsaKeyPair2 = new PgpKeyPair(PublicKeyAlgorithmTag.RsaGeneral, rsaKp, DateTime.UtcNow);

            var keyRingGen = new PgpKeyRingGenerator(PgpSignature.PositiveCertification,
                                                     rsaKeyPair1, "test", SymmetricKeyAlgorithmTag.Aes256, passPhrase,
                                                     null, null, random);
            ...
            keyRingGen = new PgpKeyRingGenerator(PgpSignature.PositiveCertification,
                                                 rsaKeyPair2, "test", SymmetricKeyAlgorithmTag.Aes256, passPhrase, null,
                                                 null, random);
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub1);
            ...
            foreach (PgpPublicKeyRing pgpPub1 in pubRings.GetKeyRings())
            {
                ...
                var pgpPub2 = new PgpPublicKeyRing(bytes);
            ...
            var secretRings = new PgpSecretKeyRingBundle(_sec1);
            count = 0;
        ...
        {
            var secretRing = new PgpSecretKeyRing(_sec10);

            foreach (PgpSecretKey secretKey in secretRing.GetSecretKeys())
            {
                IPgpPublicKey pubKey = secretKey.PublicKey;
            ...
            var publicRing = new PgpPublicKeyRing(_pub10);
        ...
        {
            var pubRing = new PgpPublicKeyRing(_subKeyBindingKey);
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub2);
            ...
            var secretRings2 = new PgpSecretKeyRingBundle(_sec2);
            ...
            foreach (PgpSecretKeyRing pgpSec1 in secretRings2.GetKeyRings())
            {
                ...
                foreach (PgpSecretKey k in pgpSec2.GetSecretKeys())
                {
                    ...
                    switch (k.KeyId)
                    {
                        case -1413891222336124627L:
                        case -4049084404703773049L:
                            k.ExtractPrivateKey(_sec2Pass1);
                            break;
                        case 59034765524361024L:
                        case -6498553574938125416L:
                            k.ExtractPrivateKey(_sec2Pass2);
                            break;
                    }
                }
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub3);
            ...
            var secretRings2 = new PgpSecretKeyRingBundle(_sec3);
            ...
            foreach (PgpSecretKeyRing pgpSec1 in secretRings2.GetKeyRings())
            {
                ...
                {
                    keyCount++;
                    k.ExtractPrivateKey(_sec3Pass1);
                }
        ...
        {
            var secretRings1 = new PgpSecretKeyRingBundle(_sec4);
            int count = 0;
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub5);
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub6);
        ...
        {
            var pubRings = new PgpPublicKeyRingBundle(_pub8);
            ...
            var secretRings1 = new PgpSecretKeyRingBundle(_sec8);
        ...
        {
            var secretRings1 = new PgpSecretKeyRingBundle(_sec9);
            ...
            foreach (PgpSecretKeyRing pgpSec1 in secretRings1.GetKeyRings())
            {
                ...
                foreach (PgpSecretKey k in pgpSec2.GetSecretKeys())
                {
                    ...
                    IPgpPrivateKey pKey = k.ExtractPrivateKey(_sec9Pass);
        ...
        public void RewrapTest()
        {
            ...
            var privRings = new PgpSecretKeyRingBundle(
                new MemoryStream(_rewrapKey, false));
